...
Run once; it is not on a schedule.

    pip install pillow numpy opencv-python-headless rembg onnxruntime
    python3 scripts/make_portrait.py photo.png
    python3 scripts/embed_portrait_font.py      # inline the font afterwards

The first run downloads a ~176 MB background-removal model, once.
It lives in ~/.u2net/ and is reused on every subsequent run.

Two things decide whether the output looks good — neither is a parameter:

  * The photo. ASCII draws with shadow, not detail — about 13 brightness
    levels in total. You need:
      - Side light (a window at ~45°, everything else turned off)
      - Tight crop from chin to just above the hair
      - Real resolution — a 320px headshot fails; thin features are
        averaged away on downscale
      - Avoid flat frontal light — it renders the face as a featureless blob

  * The darkening curve (CURVE = 1.7 below). Without it the face comes out
    washed out — brows and lips dissolve into the midtones.

Optional --crop lets you trim before the background remover sees the image:
...
After generating, always run scripts/embed_portrait_font.py to inline
JetBrains Mono. The grid bakes in an advance width of 0.600 em; a viewer
whose default monospace is narrower (Consolas ≈ 0.55) would otherwise see
the portrait about 7% too narrow.
"""
import argparse
import sys
...
CURVE = 1.7                 # darkening curve — the single biggest difference-maker
...
CHAR_W   = 7.74             # 0.600 em at FONT_SIZE — keep in step
FONT_SIZE = 12.9
LINE_H   = 15
...
def build_svg(lines, cols=COLS):
    ...
    for i, line in enumerate(lines):
        ...
        # cursor block riding the wipe edge, gone once the row lands
